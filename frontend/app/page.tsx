...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (initData.user?.id) {
      fetchProfile(initData.user.id);
      fetchUsage(initData.user.id);
    }
  }, []);

  const fetchProfile = async (tgId: number) => {
    try {
      const res = await axios.get(`${API_BASE}/api/profile`, { params: { telegram_id: tgId } });
      setUser(res.data);
    } catch (e) {}
  };
  ...
  const fetchUsage = async (tgId: number) => {
    try {
      const res = await axios.get(`${API_BASE}/api/usage`, { params: { telegram_id: tgId } });
      setUsage(res.data);
    } catch (e) {}
  };

  const quickAction = (action: string) => {
    WebApp.openTelegramLink(`https://t.me/${WebApp.initDataUnsafe.user?.username || 'yourbot'}?start=${action}`);
  };
    ...
    <div className="p-4 max-w-xl mx-auto pb-20">
          ...
          {['AI Replies', 'Roleplay', 'Files', 'Image Edits'].map((label, i) => (
            <div key={i} className="bg-[#12052A] p-4 rounded-xl">
              ...
              <div className="text-2xl font-mono mt-1">
                {usage ? `${usage.ai_remaining || 10}/10` : '--'}
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-8">
        {[
          ...
          <button key={i} onClick={() => quickAction(act.action)} className="card p-6 hover:scale-105 transition flex flex-col items-center gap-3">
      ...
      <div className="card p-6">
        ...
        {['Better Search', 'Video Generation', 'Coding Assistant'].map((f, i) => (
          <div key={i} onClick={() => WebApp.openTelegramLink('https://t.me/yourbot?start=premium')} className="flex justify-between items-center py-3 border-b border-[#1A0D3D] last:border-none cursor-pointer">
            <span>{f}</span>
            <span className="text-[#A855F7] text-sm">Locked →</span>
          </div>
        ))}
      </div>
    </div>
